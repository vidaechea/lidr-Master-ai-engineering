...
```python
# Importar escenarios
from tests.stress.scenarios import ProjectGrowthScenario

# Importar métricas determinísticas
from tests.evals.deterministic_metrics import MemoryDriftMetric, LatencyBudgetMetric, CostBudgetMetric

# Importar desde CLI
from tests.stress.runner import main
```
...
# Guarda en: tests/evals/stress/results/growth.json

uv run -m evals.stress.runner --scenario all --json results.json
# Guarda en: tests/evals/stress/results/results.json
```
...
```
┌─────────────────────────────────────────────────────┐
│        tests/evals/stress/scenarios.py                    │
...
└──────────────────┬──────────────────────────────────┘
                   │
                   ├──→ tests/evals/stress/runner.py (CLI)
                   │         │
                   │         └──→ tests/evals/stress/results/
                   │              - growth.json
                   │              - pivot.json
                   │              - contradiction.json
                   │              - aggregated.json
                   ...
                   └──→ tests/test_stress_scenarios.py (pytest)
                        │
                        └──→ tests/evals/stress/metrics/stress_metrics.py
                             - MemoryDriftMetric
...
├── stress_results.json
├── tests/evals/test_stress_scenarios.py
├── tests/evals/metrics_stress.py
...
|---------|-------|---------|
| **Organización** | Archivos dispersos | Todo en `tests/evals/stress/` |
| **Resultados** | 4 archivos en raíz | Centralizados en `results/` |
| **Tests** | Repartidos en `tests/evals/` | Coubicados en `stress/tests/` |
| **Métricas** | Separadas en `tests/evals/` | Agrupadas en `stress/metrics/` |
| **Documentación** | Mezcla en raíz y `tests/evals/stress/` | Centralizadas en `stress/docs/` |
| **Discoverabilidad** | Difícil de ubicar | `tests/evals/stress/` es punto único |
...
1. **Usar la nueva estructura en CI/CD:**
   ...
   - name: Run stress tests
     run: pytest tests/evals/stress/tests/ -m slow
   ```
...
2. **Versionar resultados:**
   ...
   # Guarda en: tests/evals/stress/results/results_20260523.json
   ```
