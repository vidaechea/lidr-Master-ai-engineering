...
Usage
-----
  uv run evals/run.py actor           # Test standard estimation
  uv run evals/run.py acb             # Test ACB pipeline
  uv run evals/run.py acb --verbose   # Show detailed output
  uv run evals/run.py --help          # Show all options
"""
...
class EvalRunner:
    ...
    def run(self, mode: Literal["actor", "acb"]) -> int:
        ...
        if mode == "actor":
            test_target = "tests/eval/test_llm_judge.py"
            ...
            test_target = "tests/eval/test_acb_quality.py"
...
Mode: acb
  File: tests/eval/test_acb_quality.py
