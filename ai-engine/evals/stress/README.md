...
```bash
bash evals/stress/run.sh --scenario all
bash evals/stress/run.sh --scenario growth --verbose
```
...
pytest -m "slow and llm_live" tests/eval/test_stress_scenarios.py

# Run a single test
pytest tests/eval/test_stress_scenarios.py::test_project_growth_scenario -m "slow and llm_live"

# With coverage
pytest tests/eval/test_stress_scenarios.py --cov=evals --cov-report=html -m "slow and llm_live"
```
...
```python
from tests.eval.metrics_stress import MemoryDriftMetric
...
uv run evals/stress/runner.py --scenario all
```
...
- [MemoryDriftMetric](../../tests/eval/metrics_stress.py#L17)
